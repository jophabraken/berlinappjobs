(function () {
  ...
  const T = {
    en: {
      ...
      faq: [
        ['How current are the listings?', 'Job feeds are re-checked daily, straight from each company\'s applicant tracking system (Greenhouse, Personio, Ashby, Lever and others). The "feeds checked" date shows the last run.'],
    ...
    de: {
      ...
      faq: [
        ['Wie aktuell sind die Stellen?', 'Die Feeds werden täglich direkt aus den Bewerbungssystemen der Unternehmen (Greenhouse, Personio, Ashby, Lever u.a.) neu geladen. Das Datum "Feeds geprüft" zeigt den letzten Lauf.'],
  ...
  let JOBS = [];
  COS.forEach((c, ci) => { c.ci = ci; c.city = c.city || 'Berlin'; (c.jobs || []).forEach((j, ji) => JOBS.push({ ...j, c, ji })); });   // ji: position in c.jobs (several jobs can share one URL)
  JOBS.forEach(j => {
  ...
  function jobRow(j, spon) {
    ...
    return `<div class="job${spon ? ' spon' : ''}" data-ci="${j.c.ci}" data-ji="${j.ji}" data-ju="${esc(j.u)}"${pg ? ` data-pg="${esc(pg)}"` : ''} ${spon ? `data-spdev="${esc(j.c.n)}"` : ''}>
      ${icoHtml(j.c)}
      <div class="jbody">
        <div class="jtop">
